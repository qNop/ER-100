...
int SysMath::getWeldNum(FloorCondition *pF,int *weldCurrent,float *weldVoltage,float *weldFeedSpeed,float *swingSpeed,float *weldTravelSpeed,float *weldFill,float *s,int count,int weldNum,int weldFloor,QString *status){
    ...
    if(weldStyleName=="立焊"){
        if(gasValue){//MaG脉冲不能摆动太快了
            ...
            //最大摆频1200  900   8之后 开始衰减 到 20 900  8 -》1200
            // 1300=8*x+Y
            // 900=18*x+Y  ->400=10X ->X=-40 ->Y=1620 最小900
            temp=pF->swingLength>8?qMax(1620-40*pF->swingLength,float(900)):WAVE_MAX_VERTICAL_SPEED;
        }
    }else
        temp=getSwingSpeed(pF->swingLength/2,pF->swingLeftStayTime,pF->swingRightStayTime,*weldTravelSpeed,WAVE_MAX_SPEED,&swingHz);
    //判断返回数据
...
int SysMath::getWeldFloor(FloorCondition *pF,float *hused,float *sused,float *weldLineYUesd,int *currentFloor,int *currentWeldNum){
    ...
    if((gasValue)&&(pulseValue)&&(wireTypeValue==0))// MAG 脉冲 实芯
        tempHeight=0;
    else if((!gasValue)&&(!pulseValue)&&(wireTypeValue==4))// CO2非脉冲 药芯
    ...
    for(i=0;i<weldNum;i++){
        ...
        *(weldLineY+i)=float(qRound(10*(*weldLineYUesd+tempHeight)))/10;
        ...
        if(grooveDirValue)//非坡口侧
            *(weldLineX+i)=-float(qRound(10*((reSwingRightLength+pF->swingLength/2+(pF->swingLength+pF->weldSwingSpacing)*(i)-(*hused+tempHeight-rootFace)*grooveAngel1Tan)-rootGap/2)))/10;
        else
            *(weldLineX+i)=float(qRound(10*((reSwingRightLength+pF->swingLength/2+(pF->swingLength+pF->weldSwingSpacing)*(i)-(*hused+tempHeight-rootFace)*grooveAngel2Tan)-rootGap/2)))/10;
        ...
        if((pF->name=="ceramicBackFloor")&&(weldStyleName!="横焊")){
            ...
            if(!grooveDirValue){
                ...
                *(startArcX+i)=rootGap/2+(pF->height/2-rootFace)*grooveAngel1Tan;
                //*(startArcX+i)=*(weldLineX+i)+(qMax(pF->height/2-rootFace,float(0)))*grooveAngel1Tan+pF->swingLength/2;
            }else{
            ...
            *(startArcY+i)=float(qRound(10**(startArcY+i)))/10;
            ...
            /*  if(grooveDirValue)//从两边起弧不从中间起弧
                *(startArcX+i)=*(weldLineX+i)+pF->swingLength;
            else
                *(startArcX+i)=*(weldLineX+i)-pF->swingLength;
             *(startArcX+i)=float(qRound(10**(startArcX+i)))/10; 以后在意 刘斌无法解决*/
        ...
        *(startWeldLineZ+i)=returnWay==0?
                    i==0?(*currentFloor-1)*startArcZz:*(startWeldLineZ+i)+(*currentFloor-1)*startArcZz+i*startArcZx:  //单程
                         *currentFloor%2?  //往返
                                           i==0?(*currentFloor-1)*startArcZz:*(startWeldLineZ+i)+(*currentFloor-1)*startArcZz+i*startArcZx:  //单程
                                                i==0?weldLength+(*currentFloor-1)*stopArcZz+i*stopArcZx:weldLength+*(startWeldLineZ+i)+(*currentFloor-1)*stopArcZz+i*stopArcZx; //偶数层
        //计算 行走方向收弧位置
        *(stopWeldLineZ+i)=returnWay==0?
                    i==0?weldLength+(*currentFloor-1)*stopArcZz+i*stopArcZx:weldLength+*(stopWeldLineZ+i)+(*currentFloor-1)*stopArcZz+i*stopArcZx //单程
                       :*currentFloor%2?
                             i==0?weldLength+(*currentFloor-1)*stopArcZz+i*stopArcZx:weldLength+*(stopWeldLineZ+i)+(*currentFloor-1)*stopArcZz+i*stopArcZx //单程
                                :i==0?(*currentFloor-1)*startArcZz+i*startArcZx:*(stopWeldLineZ+i)+(*currentFloor-1)*startArcZz+i*startArcZx;
    ...
    for(i=0;i<weldNum;i++){
        ...
        *currentWeldNum=*currentWeldNum+1;
        ...
        //横焊
        if(((weldStyleName=="横焊")&&((pF->name!="bottomFloor")&&(pF->name!="ceramicBackFloor")))||(weldStyleName=="水平角焊")||(pF->swingLength<3)){
            pF->swingLength=0;
        }
        value.clear();
        //全部参数计算完成
        value<<status<<QString::number(*currentWeldNum)<<QString::number(*currentFloor)+"/"+QString::number(i+1)<<QString::number(*(weldCurrent+i))
           ...
           <<QString::number(pF->swingLength/2)
    ...
    if(value.count()){
        if(weldStyleName!="水平角焊"){
            ...
            if(weldConnectName=="T接头"){
                ...
                b=grooveHeight/qCos(angel2*PI/180)+grooveHeightError; //立板高度+脚长
