...
void horizontalMath::firstFloorFunc(){
    ...
    //保留一位整数 小数位为偶数
    float swingLengthOne=float(qRound((swingLength-bottomFloor->swingLeftLength-bottomFloor->swingRightLength)*5))/5;
    //计算摆动频率
...
;
    ...
    swingLengthOne=float(qRound((swingLength-bottomFloor->swingLeftLength-bottomFloor->swingRightLength)*5))/5;
    //计算摆动频率
    int swingHz=getSwingHz(qCeil(swingLengthOne),0,bottomFloor->totalStayTime);
    //计算电压
    float voltage=getVoltage(current);
