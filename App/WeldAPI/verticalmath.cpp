...
//打底层函数计算
void verticalMath::firstFloorFunc(){
    QStringList value;
...
//计算第二层
void verticalMath::FloorFunc(FloorCondition *pF){
    int i,j;
...
void verticalMath::topFloorFunc(){
    /**计算盖面层***********************************************/
    int j;
    QStringList value;
    float ba=1.5;
...
int  verticalMath::getSwingHz(int swing,int floor,float stayTime){
    Q_UNUSED(floor);
    int swingHz=0;
    if(stayTime>1.21){
        status="停留时间不在范围内！";
...
float verticalMath::getVoltage(int current){
    float voltage=18;
...
int verticalMath::getFeedSpeed(int current){
    int feedspeed;
...
//分层
int verticalMath::solveN(float *pH){
    float tempH,tempHav;
...
void verticalMath::setGrooveRules(QStringList value){
    //数组有效
    if(value.count()){
