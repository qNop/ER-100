...
void VirtualKeyboardInputContext::hideInputPanel()
{
    if(d->Visible){
        ...
        if(d->Flickable){
            ...
            //搜寻对话框
        QQuickItem* item=d->Flickable->parentItem();
          while(item){
                if(item->objectName()=="Dialog"){
                    //先将VIEW强制对焦 剥夺 Textfeild 焦点
                    item->childItems().at(0)->forceActiveFocus();
                    //再将焦点归还 Dialog
...
void VirtualKeyboardInputContext::setFocusObject(QObject *object)
{
    ...
    d->InputEngine->setPlaceStr(TextInput->text());
